import { WOW_CLASS_SPEC_TO_KEY } from '../components/EventView/v0/wowClassSpecKeys';

export type V0PartyRole = 'tank' | 'healer' | 'meleeDps' | 'rangedDps';

/** Aligné sur EditParticipantDialogV0 (aperçu inscription type v0 join page). */
const SPEC_ROLES: Record<string, Record<string, V0PartyRole>> = {
  'Death Knight': {
    Blood: 'tank',
    Frost: 'meleeDps',
    Unholy: 'meleeDps',
...
function findSpecLabelForApiKey(
  ...
  const candidates = Array.from(
    new Set(
      [wowClass, normalizedClassName(wowClass), 'Death Knight', 'Deathknight'].filter(
        Boolean,
      ),
    ),
  );
...
export function v0RoleFromApiSpecialization(
