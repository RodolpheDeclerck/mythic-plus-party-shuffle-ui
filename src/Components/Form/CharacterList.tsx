...
const CharacterList: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const socketIo = io('http://localhost:8080'); // URL de ton backend WebSocket
        setSocket(socketIo);
    ...
    const fetchParties = async () => {
        try {
            const response = await axios.get<Party[]>('http://localhost:8080/api/parties');
    ...
    const handleShuffle = async () => {
        try {
            const response = await axios.get<Party[]>('http://localhost:8080/api/parties/shuffle');
